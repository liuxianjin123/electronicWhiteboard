      ...
      <template #default>
        <vxe-form :data="formData3" title-align="right" title-width="100" >
            <vxe-form-item field="age" span="24" :item-render="{}">
                <vxe-input v-model="serchG" type="text" placeholder="通过关键字检索选项" clearable @input="ddd"></vxe-input>
            </vxe-form-item>
            <vxe-form-item field="gdlx" span="24" :item-render="{}">
              <template>
                <vxe-checkbox-group v-model="formData3.gdlx">
                   <template v-if="sexList!=null">
                       <vxe-checkbox v-for="(item,index) in sexList" :key="index" :label="item.label">{{item.value}}</vxe-checkbox>
                    </template> 
                </vxe-checkbox-group>
              </template>
            </vxe-form-item>
            ...
            <vxe-form-item align="center" span="24">
              <vxe-button status="success" @click="enterToLogin" content="确 定"></vxe-button>
            </vxe-form-item>
          </vxe-form>
      </template>
    </vxe-modal>
...
export default {
      ...
      gridOptions: {//整个数据表格的配置集合
        ...
        formConfig: {//顶部的操作表单
          ...
          items: [//form各项成员
            ...
            { field: 'jlsj', title: '录入时间', span: 6, itemRender: { name: '$input', props: { placeholder: '请选择时间' } } },
            { span: 12, align: 'right', collapseNode: true, itemRender: { 
              name: '$buttons', 
              children: [
                  { props: { type: 'button', content: '查 询', status: 'primary'} },
                  { props: { type: 'button', content: '保 存', status: 'primary' },events:{click:this.saveEvent} },//保存按钮
                  { props: { type: 'button', content: '打 印' ,status: 'success'},events:{click:this.printTable}  }//打印
                ] 
              }
            }
          ]
  ...
  methods: {
    ...
    enterToLogin(){//模态框点击确定之后为触发的单元格赋值
      this.showModel=false;//隐藏模态框
      this.editRow.gdlx = this.formData3.gdlx.toString();//被编辑行的单元格赋值
    },
    async saveEvent () {//保存数据
      const $table = this.$refs.xTable;//获取到当前表格
      const { insertRecords, removeRecords, updateRecords } = $table.getRecordset();//表格：新增，删除，更新的数据
      let datas = [];
      if(insertRecords.length!=0){//获取到新增的数据
        for(var i in insertRecords){
      ...
      if(updateRecords.length!=0){//获取到更新的数据
        for(var i in updateRecords){
      ...
      if(datas.length!=0){
        ...
        const res=await  post('/Api/FormHandler.ashx/SaveFormData',datas);
        if(res.code=='0' && res.result.isSuccess){
          this.loadTable=false;
          this.$XModal.message({ content: res.result.msg, status: res.result.isSuccess?'success':'warning' })
        }
      }else{//啥数据都没抓取到就表示还没操作表格
        this.$XModal.message({ content:"未添加或修改数据", status: 'warning' })
      }
    },
    contextMenuClickEvent ({ menu, row, column,columnIndex }) {//右键菜单方法集合
      switch (menu.code) {
        case 'copy'://复制
    ...
    async delRow(row){//删除当前行数据
      ...
      if(row.id==""){//行数据ID字段是空的说明这行数据是用户新增的还没保存的数据，所以直接删除就是
        $table.remove(row)
    ...
    roleChangeEvent({ row }) {//弹窗选择事件
      this.editRow = row;//记录点击的行
      this.showModel=true;//弹出模态框
    },
    async getBrData() {//读取当前病人已有的数据
      const res=await  get('/Api/FormHandler.ashx/GetFormDataLists',{
        zyh:'0431202105139731',//病人住院号
    ...
    mockList(size) {//当这个病人一行数据都没得的时候就曾一行数据给他，方便右键
      const list = [];
    ...
    insertEvent(row,columnIndex) {//新增或复制数据
      let xTable = this.$refs.xTable;
      var record=null;
      if(row!=null){//暂时卵的，因为表格实例中唯一id复制重复了，编辑时候出现问题！
        row.id="";
        row.cUserName=this.czy.userName;
        row.cUser=this.czy.userId;
        record = row;
      }else{
        record = {//新增数据，常用的字段必须加上才行
          id:'',
          jlsj: this.GetDateTime(),
          cDate:this.GetDateTime(),
    ...
    cellClickEvent({ column, rowIndex,row }) {//监听行数据编辑前的点击事件
      localStorage.rowIndex = rowIndex;
      if(row.cUser == this.czy.userId){//判断当前被点击行数据中的创建人员和当前操作员id是否一致。
        console.log(`当前用户`+this.czy.userId+`/`+this.czy.userName+`编辑了记录时间：${row.jlsj}的${column.title}项目`);
      }
    },
    cellContextMenuEvent({ row, rowIndex }) {//鼠标右键了哪一行数据就让哪一行添加被选中的样式在这里控制操作按钮的状态
      if(row.cUser!=this.czy.userId){//创建人员和当前操作员id一致则可删除&审批，否者禁止，并并用右键相关按钮
    ...
    scrollEvent({ scrollTop, scrollLeft }) {//监听滚动事件，记录填写数据时滚动的高度，后面刷新表格之后会自动滚动到最后被点击的行的位置
      localStorage.tableScrollTop = scrollTop;
    },
    activeRowMethod ({ row }) {//判断权限
      if (row.cUser !== this.czy.userId) {//判断当前被点击行数据中的创建人员和当前操作员id是否一致，由此判断行事件
        return false
      }
      return true
    },
    async editDisabledEvent ({ row, column }) {//判断当前被点击行数据中的创建人员和当前操作员id是否一致，由此判断行事件
      this.$XModal.message({ content: '权限不足,只能由填写人本人或者护士长修改或删除当前记录!', status: 'warning' })
    },
    GetDateTime() {//获取当前时间
        var d, s;
        d = new Date();
    ...
    async czyAndBrxx(){//获取操作员和病人的基本信息
      var userData = {
        Account:'0225',
        Password:'1234'
      }
      const czy=await post('/Api/FormHandler.ashx/IsLogin',userData);
      const brxx=await get('/Api/FormHandler.ashx/GetPatientInfo',{zyh:'0431202105139731'});
      this.czy=czy.result;
      this.brxx=brxx.result;
      console.log(this.czy,this.brxx)
    }
  },
  created() {
    this.czyAndBrxx();
...
.container>>>.ant-spin-nested-loading,.container>>>.ant-spin-container {
    height: 100%;
}
