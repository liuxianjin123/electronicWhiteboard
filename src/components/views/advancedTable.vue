      ...
      <template #default>
        <vxe-form :data="formData3" title-align="right" title-width="100" >
            <vxe-form-item field="age" span="24" :item-render="{}">
                <vxe-input v-model="serchG" type="text" placeholder="通过关键字检索选项" clearable ></vxe-input>
            </vxe-form-item>
  ...
  methods: {
    ...
    async saveEvent () {//保存数据
      ...
      if(datas.length!=0){
        ...
        const res=await  this.$axios.post('/Api/FormHandler.ashx/SaveFormData',datas);
    ...
    async getBrData() {//读取当前病人已有的数据
      const res=await  this.$axios.get('/Api/FormHandler.ashx/GetFormDataLists',{
    ...
    async czyAndBrxx(){//获取操作员和病人的基本信息
      ...
      const brxx=await get('/Api/FormHandler.ashx/GetPatientInfo',{zyh:'1083202106116367'});
      this.czy=czy.result;
