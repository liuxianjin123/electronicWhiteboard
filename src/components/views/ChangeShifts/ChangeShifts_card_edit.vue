<template>
    <div style="height: 550px;font-size: 16px;">
        <div class="fz">
            <!-- {{dataList}} -->
            <span class="xin"> * </span>
             分组名称： <input type="text" autocomplete="off" v-model="group_name" @input="input_blur($event)"  placeholder="请输入分组名称" name="group_name">
    ...
    methods: {
        ...
        group(array, subGroupLength) {//数组，分割长度  分割数组，生成新数组
            ...
            while(index < array.length) {
                newArray.push({'children':array.slice(index, index += subGroupLength),title:index-subGroupLength+'——'+index+'床',key:index-subGroupLength+'——'+index,name:'title'});
            }
            return newArray;
        },
        sortBykey(ary, key) {
       ...
       async EditSave(){//保存
            ...
            if(this.preservation[0].groupName.length<2){
                ...
                const res = await this.$axios.post('/han/handover/saveGroup',this.preservation[0]);
                if(res.result){
...
.Tube_bed>>>.ant-transfer-list:first-of-type .ant-transfer-list-header-selected>span:first-of-type{
    display: none;
}
</style>