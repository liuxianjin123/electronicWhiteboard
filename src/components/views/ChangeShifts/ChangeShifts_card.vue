    ...
    <a-row :gutter="16">
      <a-col :span="6" v-for="(group, index_g) in groups" :key="index_g">
        <div class="cards" style="width: 90%" :span="8" :offset="8">
          <a-card :title="group.groupName" style="width: 100%">
            <a-row type="flex" class="team">
              <a-col :span="4" style="line-height: 80px">
                <img
                  src="../../../../static/images/ChangeShifts/zhuyuan.png"
                  width="40px"
                  alt=""
                />
              </a-col>
              <a-col
                :span="20"
                style="padding-top: 5%; height: 80px"
                @mouseleave="more_h()"
              >
                <more
                  :dataList="group.groupMembers"
                  :datatitle="group.groupName"
                  v-show="'a1' + index_g == int"
                  @mouseleave="more_h()"
                ></more>
                <div
                ...
                >
                  <div
                    class="team_people"
                    v-show="groupMembers.isExist"
                    v-for="(groupMembers, index) in beyond_hidden(
                      group.groupMembers,
                      8
                    )"
                    :key="index"
            ...
            <a-row type="flex" class="bed">
              <a-col :span="4" style="line-height: 150px">
                <img
                  src="../../../../static/images/ChangeShifts/bc.png"
                  width="40px"
                  alt=""
                />
              </a-col>
              <a-col
                :span="20"
                style="padding-top: 5%; height: 150px"
                @mouseleave="more_h()"
              >
                <more
                  :dataList="group.beds"
                  :datatitle="group.groupName"
                  v-show="'a2' + index_g == int"
                  @mouseleave="more_h()"
                ></more>
                <div
                  v-if="group.bedCount > 15"
                  v-show="'a2' + index_g != int"
                  class="yingchang"
                >
                  <div
                    class="bed_number"
                    v-show="beds.isExist"
                    v-for="(beds, index) in group.beds"
                    :key="index"
                  >
                  ...
                  <div class="more" @mouseenter="more('a2' + index_g)">
                    更多...
                  </div>
                </div>
                <div v-else>
          ...
          <a-row type="flex">
            <a-col :flex="1" class="buttom_edit" @click="showModal(index_g)"
              >编辑</a-col
            >
            <template>
              <a-popconfirm
                :title="'是否删除' + group.groupName + '，删除将清空所有数据?'"
                ok-text="确定"
                cancel-text="取消"
...
export default {
  ...
  methods: {
    beyond_hidden(arr, number) {
      //数组超出数值隐藏 arr数组 number最大长度
