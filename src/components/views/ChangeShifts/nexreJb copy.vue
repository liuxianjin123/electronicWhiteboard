  ...
  methods: {
    ...
    async getAlldeptData(){
      ...
      if(this.$store.state.createJbsj){
        res = await get('/han/HandoverProc/GetSuccessionData/'+localStorage.handoverTime);
      }else{
        res = await get('/han/HandoverProc/GetHandoverPatients?handoverTime='+localStorage.handoverTime+'&orderOfClassesId='+localStorage.orderOfClassesId);      
      }
      if(res.result){
