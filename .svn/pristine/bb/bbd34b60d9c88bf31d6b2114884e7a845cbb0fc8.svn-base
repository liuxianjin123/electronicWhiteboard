...
export default {
  ...
  methods: {
    ...
    async getTopItemData(type){
      const res = await this.$axios.get('/han/HandoverProc/GetPatientNumber/'+type);
      if(res.result){
    ...
    async getAlldeptData(){
      ...
      if(this.$store.state.createJbsj){
        res = await this.$axios.get('/han/HandoverProc/GetSuccessionData/'+localStorage.handoverTime);
      }else{
        res = await this.$axios.get('/han/HandoverProc/GetHandoverPatients?handoverTime='+localStorage.handoverTime+'&orderOfClassesId='+localStorage.orderOfClassesId);      
      }
      if(res.result){
    ...
    filterJbData(data){
      if(this.$store.state.createJbsj){//如果是生成交班数据，直接不处理返回
        return data;
      }else{//否则的话需要区分哪些属于交班的数据
        var newArr=[];
        for(var i in data){
          if(this.$store.state.isjj && !data[i].isHandover){//目前属于交班状态并且isHandover字段为false为交班
            newArr.push(data[i]);  
          }
    ...
    async insertItemData(jbDate,Bid,vals){//交班时间，班次,修改&新增的数据
      ...
      const res = await this.$axios.post('/han/HandoverProc/SaveHandoverPatients?handoverTime='+localStorage.handoverTime+'&orderOfClassesId='+localStorage.orderOfClassesId, obj);
      if(res.result){
    ...
    async submintClass(type,rowData,isPl){
      var isOk=false;//生成排班的时候判断重复数据，验证是否继续
      var obj = {};
      var datas =type;
      var title= type;
      ...
      let res;
      if(this.$store.state.isjj && !this.$store.state.createJbsj){//表示交班
        isOk=true;
        res = await this.$axios.post('/han/HandoverProc/BatchShift', obj);
      }else if(!this.$store.state.isjj && !this.$store.state.createJbsj){//接班
        isOk=true;
        res = await this.$axios.post('/han/HandoverProc/BatchSuccession', obj);
      }else if(this.$store.state.createJbsj){//生成交班
        var _this = this;
        if(obj[title].length==1){
          if(obj[title][0].whetherToGenerate){//单条数据验证是否已经被生成过了
            this.$confirm({
              title: '班次确认',
              content: '当前数据已经生成过了,确认生成会覆盖之前的当条数据,确认覆盖生成吗?',
              async onOk() {
                isOk=false;
                res = await this.$axios.post('/han/HandoverProc/CreateSuccessionShift?handoverTime='+localStorage.handoverTime+'&orderOfClassesId='+localStorage.orderOfClassesId, obj);
          ...
          if(arr.length>=1){//多条数据验证是否已经被生成过了
            this.$confirm({
              title: '班次确认',
              content: '当前数据集中的第'+arr.toString()+'条数据已经生成过了,确认生成会覆盖之前的当条数据,确认覆盖生成吗?',
              async onOk() {
                isOk=false;
                res = await post('/han/HandoverProc/CreateSuccessionShift?handoverTime='+localStorage.handoverTime+'&orderOfClassesId='+localStorage.orderOfClassesId, obj);
                _this.PreturnMsg(res,type);
              }
            });
          }
